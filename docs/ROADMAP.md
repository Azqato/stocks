...
- **Crypto** (13, up from 1): BTC, ETH, BNB, XRP, SOL, TRX, DOGE, ZEC, XLM, ADA, XMR, LTC, AVAX — **ordered by live market cap** (largest first) per explicit owner instruction, which also applies going forward: re-sort by market cap whenever this section is edited, since ranks drift over time. One correction from the owner's draft: **XLM's name fixed** from "Steller" (typo) to "Stellar". Hyperliquid was considered and rejected — owner decision (2026-07-10): not tracking it, do not revisit.
...
- **FCF Growth FWD column (MAYBE — owner tagged 2026-08-13, not scheduled).** Owner wants a **forward** free-cash-flow growth metric. **Data blocker (confirmed by probe 2026-08-13):** yfinance exposes **no forward FCF analyst consensus** — there is no FCF equivalent of the `revenue_estimate` / `earnings_estimate` tables that power Rev/EPS Growth FWD. Only trailing FCF is available (`info["freeCashflow"]` TTM, plus the historical cash-flow statement, from which trailing YoY FCF growth is computable, e.g. NVDA $60.9B→$96.7B = +58.9%). Three options were laid out: (A) trailing **FCF Growth TTM** from the statement — real but not forward; (B) a **modeled/proxy forward** figure — e.g. current FCF margin × forward revenue growth, or EPS-growth as a stand-in; (C) don't add it. **Owner chose Option B (the proxy)**, explicitly as a *maybe*, to revisit later — NOT now. Open concern to resolve before building B: a homemade proxy displayed under a "FWD" label conflicts with the site's convention that "FWD" means real analyst consensus (same principle behind the documented "EPS Growth FWD is GAAP-basis" caveat); if built, it must be clearly labeled as an estimate/derived figure, not passed off as consensus, and almost certainly ships as a **weight-0 context column** (like P/E FWD and Net Cash/Mkt Cap) so it never touches the scored model. Also note the extra per-ticker cost if any statement pull is involved (`t.cashflow` is an additional API call across ~500+ symbols). When promoted to a version: decide the exact proxy formula, confirm coverage across all universes, and pick the display label + caveat wording.
