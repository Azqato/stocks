...
- **Multi-year forward EPS and P/E table (owner-requested 2026-09-21 from a competitor screenshot, not scheduled).** A ticker-list tool, separate from the screener, showing each company's forward EPS ladder and the P/E that falls out of it at each year, so valuation compression over time is visible in one row. The reference implementation shows: a free-text ticker box with Generate, Refresh Prices, Add Ticker, and Export CSV; columns for next-year and following-year EPS, three P/E columns (current FY, next FY, following FY), a 2-year EPS dollar change and a 2-year EPS growth percent; P/E cells color-graded green to red by cheapness; and, as its real differentiator, **every EPS cell is editable and the P/E columns recalculate live**, so a reader can substitute their own estimate for consensus.
