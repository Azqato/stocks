...
| v4.2.0 — **Market Overview: bond yields, planned, not yet built.** Owner request (2026-07-10, with a CNBC "Bonds" tab reference screenshot): add bond yields as yields (a percentage plus a yield-point change), not fund price movement. Data-source probe (done, see ROADMAP.md): US Treasury yields work directly via yfinance (`^TNX`/`^TYX`/`^FVX`/`^IRX`, already in percent, no unit conversion needed); foreign sovereign yields (Bund, Gilt, JGB) are **not available** via yfinance's free Yahoo data (all attempted tickers 404'd) — a CNBC-identical multi-country curve is not achievable with this site's current data source, scope will be corrected to US Treasury yields only. Prioritized ahead of the sparklines release (renumbered to v4.3.0). Open design questions before build: which tenors, yield-card layout (distinct from the existing price-card format), section placement, color convention. Awaiting owner confirmation on scope before implementation starts | Next up | Planned |
| v4.3.0 — Screener score history sparklines (mine screener.json git history for per-stock score trends; renumbered from v4.1.0 on 2026-07-09, then from v4.2.0 on 2026-07-10 to make room for the prioritized bond-yields item) | After v4.2.0 | Planned |
| v4.4.0 — Deeper index fund coverage (sector ETFs, international allocation, bond tent strategy; renumbered from v4.2.0 on 2026-07-09, then from v4.3.0 on 2026-07-10) | TBD | Planned |
| v4.5.0 — Additional illustrative examples using historical market events (renumbered from v4.3.0 on 2026-07-09, then from v4.4.0 on 2026-07-10) | TBD | Planned |
| v4.6.0 — Additional philosophy sections (renumbered from v4.4.0 on 2026-07-09, then from v4.5.0 on 2026-07-10) | TBD | Planned |
| v4.7.0 — Conference call research guide (renumbered from v4.5.0 on 2026-07-09, then from v4.6.0 on 2026-07-10) | TBD | Planned |

### Feature Breakdown by Phase

**v3.x (current):** Screener with daily data, relative percentile scoring, methodology popup, documentation consolidation.

**Queue head (updated 2026-07-04):** v3.28.0 GVD universes, v3.29.0 rank-based tier scale, v3.30.0 scoring model v2, v3.31.0 margins-out re-weighting, v3.32.0 pipeline cleanup, v3.33.0 ETFs universe, v3.34.0 International universe, v3.34.5 workflow timing review, v3.34.6 International same-company duplicate fix, v3.34.7 International name-primary display, v3.34.8 horizontal-scroll bug fix, v3.36.0 "MAG 10" filter, v3.37.0 ETF rating methodology review, and v4.0.0 screener responsive redesign are all shipped — all seven screener universes are live, the daily pipeline is re-scheduled (Nasdaq 100 21:30 → ETFs 22:00 → S&P 500 22:30 → GVD 23:00 → International 23:30 UTC, all Mon-Fri, 30-min gaps throughout, anchored to the latest-possible market close), the International list's top 100 correctly represents 100 distinct companies, the International universe leads with company names, a fixed 10-stock "MAG 10" watchlist toggle scores against the full S&P 500, the ETF scoring model is reweighted (Yield/Expense Ratio removed, 5Y/10Y weighted up), the screener now auto-hides column groups responsively so no width ever requires horizontal scroll, and `indices.html` documents Price vs 200-Day MA as a fifth timing signal. v3.34.10's scrollbar-visibility fix is superseded; v3.35.0 is retired and merged into v4.0.0. v4.1.0 Market Overview page also shipped 2026-07-10 (`market.html`, 9th content page, own intraday feed), followed same-day by v4.1.4 (symbol expansion to 19 across 3 groups, renames, grouped-section rendering). **Next: v4.2.0** bond yields for Market Overview (planned, awaiting owner scope confirmation — see PRD milestone row and ROADMAP.md), then v4.3.0 sparklines, then v4.4.0-v4.7.0 content releases. Constituent sourcing decision (2026-07-03): index universes stay on Wikipedia (updates within days of index changes); ETF universes use Vanguard's holdings API (authoritative but month-lagged, acceptable because the fund's published holdings are the universe definition). Switching the S&P 500 to VOO holdings was considered and rejected for the freshness reason. Note the dependency: the v4.3.0 sparklines mine the git history of the committed data feeds, so keeping generated data files in the repo is an intentional design choice, not tech debt.

**v4.x (order updated 2026-07-10):** v4.0.0 screener responsive redesign & site-wide mobile-friendliness pass shipped 2026-07-04, v4.0.1 MAG 10 button spacing fix shipped same day, v4.1.1 two new philosophy sections + FAQ items shipped 2026-07-08, v4.1.2 MAG 10 button restyle and v4.1.3 sidebar rebrand (docs backfill) shipped 2026-07-09, v4.1.0 Market Overview page and v4.1.4 Market Overview expansion both shipped 2026-07-10 (owner-requested and prioritized 2026-07-09, ahead of sparklines), v4.2.0 Market Overview bond yields next (new, added 2026-07-10, awaiting owner scope confirmation), v4.3.0 score history sparklines (renumbered from v4.1.0, then v4.2.0), v4.4.0 deeper index fund coverage (renumbered from v4.2.0, then v4.3.0), v4.5.0 additional historical illustrative examples (renumbered from v4.3.0, then v4.4.0), v4.6.0 additional philosophy sections (renumbered from v4.4.0, then v4.5.0), v4.7.0 conference call research guide (renumbered from v4.5.0, then v4.6.0). Potential Growth/Value/Dividend standalone framework pages remain unversioned backlog.
