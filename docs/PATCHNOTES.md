...
- **Consolidated from 5 documentation files to 4.** `docs/ROADMAP.md` (714 lines: detailed implementation plans for every open roadmap item, plus the full as-built record for recently shipped items) was merged into `docs/PRD.md`'s Roadmap section and the file removed. Going forward, PRD.md's Roadmap section is the single source of truth for both **what** is planned and **how** it will be built; there is no longer a separate "detailed plans" file. `docs/ROADMAP.md` was never a GitHub Pages-served page (only visible via GitHub's own repo browser, not `azqato.github.io/stocks/`), so its removal is a plain delete with no redirect needed, consistent with the project's established practice for removing internal, non-served files.
- **`README.md` rewritten as a public-facing front door.** Removed the tech stack table, prerequisites, installation commands, local-serving instructions, environment variable notes, build/deploy steps, the constituent-source table, and the full project-structure tree, all moved to (or already present in, in more detail) `docs/PRD.md`'s Runbook and Technical Requirements sections. README.md now covers only: what the site is, what it offers, who it's for, current status, and where to find the rest of the documentation.
- **`docs/PRD.md`'s Screener Scoring Model section corrected to match the live v3 model.** The dedicated section (and External FAQ items #5-#7) still described the v3.31.0 three-pillar model (Growth 60/Valuation 20/Balance sheet 20) five entries after v4.1.12 (2026-08-19) shipped the current four-pillar model (TTM 25/FWD 25/Valuation 25/Balance sheet 25); the milestone table and Feature List elsewhere in the same file had been updated correctly, but this section and the FAQ had not. Rewrote both to match `screener.js`'s live `METRICS` weights, verified directly against the code in this pass.
- **`docs/PRD.md`'s ETF Universe Scoring Model section corrected.** The "Scoring criteria" table still showed the pre-v3.37.0 spec (Yield 10pts, Expense Ratio 10pts, 1Y/5Y/10Y at 10pts each) rather than the shipped result (Yield and Expense Ratio removed from scoring, 5Y/10Y raised to 20pts each); the "As built" paragraph immediately below the table already had it right. Rewrote the table to match.
- **Cron schedule references corrected to the actual odd-minute schedule.** `README.md`'s project-structure tree comments and `docs/PRD.md`'s ASCII architecture diagram and folder-structure tree both still showed the pre-v4.1.8 round-minute schedule (21:30/22:00/22:30/23:00/23:30 UTC, Market Overview 22:05, constituents Sat 23:00) more than a month after v4.1.8 (2026-07-13) moved every cron to odd, off-peak minutes specifically to stop jobs from being delayed or dropped. Verified the actual current schedule directly against every file in `.github/workflows/` and corrected both diagrams to match (21:37/22:12/22:42/23:12/23:42 UTC, Market Overview 15:07/19:07/22:07, constituents Sat 23:17); the prose descriptions elsewhere in both files already had the correct times, only the diagrams were stale.
- **`docs/DESIGN.md` updated to the current design state**, having last been touched 2026-06-27 (v2.0) and covering only through v3.23.0, a roughly seven-week, 50-plus-release gap. Added: the 5 tier color tokens to the token-count line (was still "14", palette table already had 19 including tiers); a full component-pattern subsection for `market.html`'s card-grid design language, introduced v4.1.0 and never previously documented; `market.html` to the Social Cards per-page table (it has full OG/Twitter tags in the live code; the table just never listed it); a corrected modal full-width breakpoint (1023px, not the 900px carried over from the v3.19.0 changelog entry); an extended Version History table through this entry.
- **`docs/PRD.md`'s Documentation Process section** rewritten to describe the current 4-file structure and this audit's process, replacing the "five files" description.

### Added

To `docs/PRD.md`, all previously absent: **Conventions** (naming, formatting, commit message style, derived from the code and `git log`, not assumed), **Writing Style** (formalizes the existing sitewide no-em-dash rule as the documented house style for docs/UI copy/code comments, which the project already enforced through its own Content Philosophy section), **Browser Testing** (documents the project's actual established practice, headless Chrome, referenced in dozens of prior PATCHNOTES verification notes, and flags that this differs from this audit template's own Edge-by-default guidance, per instruction to keep an existing practice over the template default), **Deprecation and Removal** (documents the project's actual practice, evidenced by `.gitignore`'s "Orphaned files" block: a plain delete with the old filename permanently gitignored, no redirect stub, since the project has no redirect mechanism, being a static GitHub Pages site with no server-side rewrites), **Documentation Versus Reality** (a table of every discrepancy found in this audit, resolved or open), **Risks and Open Questions** (fragile areas, an undefined-CSS-token bug found in `style.css`, and the roadmap's existing open questions consolidated into one place), **Working Practice** (a "where to look first" table and concrete dos/don'ts for future contributors, human or AI).

### Fixed

- Two internal `docs/DESIGN.md` self-contradictions: the palette table listed 19 color tokens while the CSS File Structure section's summary line still said "14"; and the modal-width note cited a breakpoint (900px) that no longer matches the current `screener.html` (1023px).
- One internal `docs/PRD.md` self-contradiction: the Feature List correctly described the v4.1.12 four-pillar scoring model while the dedicated Screener Scoring Model section and two FAQ answers in the same file still described the retired three-pillar model (see Changed, above).

### Removed

- `docs/ROADMAP.md` (714 lines), merged into `docs/PRD.md`'s Roadmap section per the Changed note above. Not a public-facing removal; see that section for why no redirect was needed.

Full findings, the resolution for each, and the working practice for keeping these four files in sync going forward are recorded in `docs/PRD.md` under Documentation Versus Reality and Working Practice.
